...
import RenderReplies from "../ProfilePage/RenderReplies";
...
function HomeFeed() {
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      try {
        const response = await axios.get(`/user/post/${url}`);
        if (response.status === 200) {
          const data = response.data;
          setPost(data);
          setIsLoading(false);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const id = window.localStorage.getItem("userId");
      const userId = JSON.parse(id).id;
      const response = await axios.post(`/user/post/${userId}`, {
        id: id,
        postText: text,
      });
      if (response.status === 200) {
  ...
  const toggleLikeStatus = async (postId, commentId = "", replyId = "") => {
    ...
    if (updatedPost) {
      const userLikedPost = updatedPost.likes.some(
        (like) => like.userId === JSON.parse(User).id
      );
      ...
      if (userLikedPost) {
        ...
        updatedPost.likes.push({ userId: JSON.parse(User).id });
      }
        ...
        try {
          ...
          if (response.status === 200) {
            if (userLikedPostComment) {
              ...
              updatedComment.likes.push({ userId: JSON.parse(User).id });
            }
          } else {
            console.error("Request failed");
            ...
            <ListItemText
              ...
              secondary={
                <div>
                  ...
                  <div>{formatDate(reply.dateCreated)}</div>
  ...
  const updatePosts = (posts) => {
    setPost([...posts]);
    console.log("Updating posts method called.");
  };
  ...
  return (
    <Container
      ...
      className={`fade-in ${showElement ? "show" : ""}`}
    >
      <Logout />
      <Box sx={{ flexGrow: 1 }}>
        <AppBar
          position="static"
          sx={{ margin: "5%", flexDirection: "row" }}
          id="search-box"
        >
          <Toolbar sx={{ margin: "2%" }}>
            <form>
              ...
              <TextareaAutosize
                style={{ resize: "none", minWidth: "50%", minHeight: "20px" }}
                onChange={(e) => {
      ...
      <Paper
        elevation={3}
        sx={{ padding: "3%", marginBottom: "2%", width: "100%" }}
        id="Feed-Post-Submit-Post"
      >
        <Typography component="span" variant="h4" sx={{ marginRight: "10px" }}>
          Post
        </Typography>
        <TextareaAutosize
          id="textField"
        ...
        <Button
          ...
          sx={{
            ...
            padding: "1px",
            ":disabled": {
              backgroundColor: "#9e9e9e", // Light grey when disabled
            },
            ":hover": {
              backgroundColor: "#7d9a68",
            },
          }}
        >
          Submit
          ...
          {post &&
            post.map((post, index) => (
              <div>
                ...
                >
                  {JSON.parse(User).id === post.userId && (
                    <IconButton
                      style={{
                        position: "absolute",
                        top: "5px",
                  ...
                  <ListItem>
                    <ListItemText
                      ...
                      secondary={
                        ...
                        >
                          {post.content}{" "}
                          {/* Potentially put this into a Paper */}
                        </Typography>
                      }
                    />
                  </ListItem>
                  <div style={{ marginLeft: "1%" }}>
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={() =>
                          AddComment(JSON.parse(User).id, post._id)
                        }
                      >
                        Comment
                      </button>
                        ...
                        <div>
                          ...
                          <Typography className="post">
                            ...
                            <Typography sx={{ right: 0, bottom: 0 }}>
                              {formatDate(postToDeleteContent.date)}
                            </Typography>
                          </Typography>
                        </div>
                      )}
